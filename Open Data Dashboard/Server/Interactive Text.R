...
Results show that most practices behave as expected, forming a clear central group, while a small number stand out with much higher or lower activity than predicted. These outliers are flagged for further review and provide an early warning signal rather than evidence of inappropriate prescribing. If the model doesn't appear right away, it can take up to a minute to appear. If the model ever disappears, press the button again and it will reappear."
  ...
  }  else if (input$AI_Model_Version == "Main_Model") {"characteristics (such as practice size, population age profile, location, and time of year) to estimate how many items a GP practice would normally be expected to prescribe. It then compares these expected values with what actually happened, highlighting practices where the difference is unusually large.
